...
// Test-only copy of the real, already-deployed $INAYA token source (root-level InayaToken.sol,
// which Hardhat's default `sources: "./contracts"` path never compiles) so the bridge test
// suite can deploy a fresh instance with IDENTICAL fee/cap behavior instead of substituting a
// fee-free MockINAYA -- specifically to exercise the bridge's fee-buffer accounting, which only
// matters against a token that actually charges a transfer fee. Kept byte-for-byte identical to
// the root file; the root file (and the real on-chain deployment it mirrors) is the source of
