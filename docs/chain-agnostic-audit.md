...
- **Solana Devnet**: program deployed (`76KGt54jrh142nibdFH9BtZHxSu68rrDwxCTp5d98kZn`), **not wired on-chain** — `initialize`/`add_trusted_chain`/`set_home_addresses` never called. Source of truth: `deployments/bridge/solanaDevnet.json`'s own notes (a stale claim in the program's `lib.rs` doc comment says otherwise — the deployment JSON is authoritative).
