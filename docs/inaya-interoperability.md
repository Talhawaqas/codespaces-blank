...
- [x] At least 3 additional ecosystems technically demonstrated — **3 of 3, real and proven**: BSC → Ethereum (Sepolia), BSC → Arbitrum (Arbitrum Sepolia), BSC → Avalanche (Fuji), every one a full end-to-end lock + complete + verified-balance transfer.
- [~] Solana / Sui / Aptos — attempted beyond the required 3, each hit a real, DEFINITIVELY-diagnosed external blocker (not an Inaya bug, not fixable by more code here): Solana's wrapped token is created and verified, but BSC was never registered as a trusted source chain on Wormhole's own Solana Devnet Token Bridge (a guardian-governance action only Wormhole can perform). Sui and Aptos both fail inside the Wormhole SDK itself — Aptos with an explicit, unambiguous error (`publishing module bytecode version 5 is not allowed; minimum is 6`), Sui with an equivalent Move-level package-version abort — both packages' last release (2026-07-29) predates a network-side VM/version upgrade. Full root-cause detail, including the exact on-chain checks that confirmed each, in `deployments/interop/wormhole-wtt/bscTestnet-attestation.json`.

**Honest summary**: the architecture, provider selection, capability model, transfer-tracking schema, and a REAL (not stubbed) live query against Wormhole's own chain registry are done and tested — and beyond that, 3 full end-to-end testnet transfers are proven live in production (`/bridge`). The 3 further chains attempted (Solana/Sui/Aptos) are blocked by causes outside Inaya's control (Wormhole governance + Wormhole SDK staleness), each diagnosed to its exact root cause rather than left as an unexplained failure.
