// scripts/relayer-dry-run.mjs
//
// Real end-to-end relayer dry run against the LIVE BSC Testnet <-> Sepolia deployment:
// bridges real INAYA, then performs exactly what the cron/relay-messages route does
// (each configured validator signs the message hash, then the relayer wallet submits
// executeMessage on the destination chain) -- proving the relayer logic works for real,
// not just in a single-process simulation.
//
// Run with: node scripts/relayer-dry-run.mjs
...
const sepolia = JSON.parse(fs.readFileSync("deployments/bridge/sepolia.json", "utf8"));
...
async function main() {
  ...
  const spokeProvider = new ethers.JsonRpcProvider(process.env.SEPOLIA_RPC);
  ...
  const recipient = deployer.address; // send back to ourselves on Sepolia for easy verification
  const tx = await bridgeHome.bridgeOut(sepolia.chainId, ethers.zeroPadValue(recipient, 32), amount);
  ...
  console.log("Step 3: relayer submits executeMessage on Sepolia (relayer:", relayer.address, ")");
  const messengerSpoke = new ethers.Contract(sepolia.messenger, MESSENGER_ABI, relayer);
  const plainMessage = {
  ...
  const wrapped = new ethers.Contract(sepolia.wrappedToken, WRAPPED_ABI, spokeProvider);
  const balance = await wrapped.balanceOf(recipient);
  console.log("Recipient wrapped INAYA balance on Sepolia:", ethers.formatUnits(balance, 18));
