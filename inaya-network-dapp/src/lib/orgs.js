...
export async function getOrgCollections() {
  ...
  return {
    ...
    nasGroups: db.collection("nasGroups"),
    nasPools: db.collection("nasPools"),
    nasSnapshots: db.collection("nasSnapshots"),
    nasSnapshotPolicies: db.collection("nasSnapshotPolicies"),
    nasBackupPolicies: db.collection("nasBackupPolicies"),
    nasBackupIndex: db.collection("nasBackupIndex"),
    nasReplicationPolicies: db.collection("nasReplicationPolicies"),
    nasCloudTargets: db.collection("nasCloudTargets"),
    nasThreatEvents: db.collection("nasThreatEvents"),
    nasTieringProposals: db.collection("nasTieringProposals"),
    nasJobs: db.collection("nasJobs"),
    nasEvidence: db.collection("nasEvidence"),
    nasUpdates: db.collection("nasUpdates"),
    nasStateCommitments: db.collection("nasStateCommitments"),
    nasAcls: db.collection("nasAcls"),
    nasRequests: db.collection("nasRequests"),
    nasApplianceState: db.collection("nasApplianceState"),
...
export async function ensureOrgIndexes() {
  ...
  await Promise.all([
    ...
    nasGroups.createIndex({ orgId: 1, applianceId: 1, name: 1 }, { unique: true, partialFilterExpression: { deletedAt: null } }),
    nasPools.createIndex({ orgId: 1, applianceId: 1, name: 1 }, { unique: true, partialFilterExpression: { deletedAt: null } }),
    nasSnapshots.createIndex({ orgId: 1, shareId: 1, createdAt: -1 }),
    nasSnapshots.createIndex({ shareId: 1, name: 1 }, { unique: true }),
    nasSnapshotPolicies.createIndex({ orgId: 1, shareId: 1 }, { unique: true }),
    nasBackupPolicies.createIndex({ orgId: 1, shareId: 1 }, { unique: true }),
    nasBackupIndex.createIndex({ orgId: 1, shareId: 1, targetKey: 1, relativePath: 1 }, { unique: true }),
    nasReplicationPolicies.createIndex({ orgId: 1, shareId: 1, createdAt: -1 }),
    nasCloudTargets.createIndex({ orgId: 1, deletedAt: 1 }),
    nasThreatEvents.createIndex({ orgId: 1, shareId: 1, detectedAt: -1 }),
    nasTieringProposals.createIndex({ orgId: 1, shareId: 1, createdAt: -1 }),
    nasJobs.createIndex({ orgId: 1, idempotencyKey: 1 }, { unique: true }),
    nasJobs.createIndex({ status: 1, nextAttemptAt: 1 }),
    nasEvidence.createIndex({ orgId: 1, applianceId: 1, createdAt: -1 }),
    nasEvidence.createIndex({ orgId: 1, subjectId: 1, createdAt: -1 }),
    nasUpdates.createIndex({ orgId: 1, applianceId: 1, createdAt: -1 }),
    nasStateCommitments.createIndex({ orgId: 1, applianceId: 1, createdAt: -1 }),
    nasAcls.createIndex({ orgId: 1, shareId: 1 }),
    nasRequests.createIndex({ orgId: 1, key: 1 }, { unique: true }),
    nasRequests.createIndex({ createdAt: 1 }, { expireAfterSeconds: 86400 }),
    nasApplianceState.createIndex({ orgId: 1, applianceId: 1 }, { unique: true }),
    // AI Security Workflow 2026 SOW
