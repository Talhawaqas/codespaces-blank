...
// Client-side encrypt/shard/pin pipeline for receipt attachments — same
// three pure functions business/page.js's DocumentColumn uses, duplicated
// here for the same reason that file's header comment gives (nothing to
// import: they're closures, not an exported module).
            ...
            {invoices.map((inv) => (
              <button key={inv.id} onClick={() => setSelected(inv)} className="w-full flex items-center justify-between gap-3 bg-black/20 border border-white/5 rounded-lg p-3 text-left hover:bg-white/5">
                <div className="min-w-0">
                  ...
                  <p className="text-[#94a3b8] text-[12px] font-mono mt-0.5">Due {new Date(inv.dueDate).toLocaleDateString()}</p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-white text-sm font-mono tabular-nums">${inv.total.toFixed(2)}</span>
  ...
  return (
    <Modal title="New invoice" onClose={onClose} wide>
      <form onSubmit={handleSubmit} className="space-y-3">
        <select value={departmentId} onChange={(e) => { setDepartmentId(e.target.value); setContactId(""); }} required className="w-full bg-black/45 border border-white/15 rounded-lg px-2.5 py-2 text-xs text-white">
        ...
        <input value={dueDate} onChange={(e) => setDueDate(e.target.value)} type="date" required className="w-full bg-black/45 border border-white/15 rounded-lg px-3 py-2 text-sm text-white" />

        <div className="space-y-2 border-t border-white/5 pt-3">
          <p className="text-[11px] font-bold uppercase text-[#94a3b8]">Line items</p>
          {lineItems.map((item, i) => (
            <div key={i} className="grid grid-cols-[1fr_60px_80px_24px] gap-1.5">
              <input value={item.description} onChange={(e) => updateItem(i, "description", e.target.value)} placeholder="Description" className="bg-black/45 border border-white/15 rounded-lg px-2 py-1.5 text-xs text-white" />
              <input value={item.quantity} onChange={(e) => updateItem(i, "quantity", e.target.value)} type="number" min="0.01" step="0.01" placeholder="Qty" className="bg-black/45 border border-white/15 rounded-lg px-2 py-1.5 text-xs text-white" />
              <input value={item.unitPrice} onChange={(e) => updateItem(i, "unitPrice", e.target.value)} type="number" min="0" step="0.01" placeholder="Price" className="bg-black/45 border border-white/15 rounded-lg px-2 py-1.5 text-xs text-white" />
              <button type="button" onClick={() => removeItem(i)} disabled={lineItems.length === 1} className="text-[#94a3b8] hover:text-red-400 disabled:opacity-30 text-lg leading-none">×</button>
            </div>
          ))}
          <button type="button" onClick={addItem} className="text-[11px] font-bold uppercase text-[#00f2fe]">+ Add line</button>
          <p className="text-right text-white text-sm font-mono tabular-nums">Total: ${total.toFixed(2)}</p>
        </div>

        <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Notes (optional)" rows={2} className="w-full bg-black/45 border border-white/15 rounded-lg px-3 py-2 text-sm text-white placeholder-[#8a96ab]" />
        {error && <p className="text-red-400 text-xs">{error}</p>}
    ...
    <Modal title={invoice.invoiceNumber} onClose={onClose}>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-white text-lg font-mono tabular-nums">${invoice.total.toFixed(2)}</span>
        </div>
        <p className="text-[12px] font-mono text-[#94a3b8]">Issued {new Date(invoice.issueDate).toLocaleDateString()} · Due {new Date(invoice.dueDate).toLocaleDateString()}</p>

        <div className="space-y-1 border-t border-white/5 pt-3">
          {invoice.lineItems.map((it, i) => (
            <div key={i} className="flex items-center justify-between text-xs">
              <span className="text-slate-300">{it.description} × {it.quantity}</span>
              <span className="text-white font-mono tabular-nums">${(it.quantity * it.unitPrice).toFixed(2)}</span>
            </div>
          ))}
        </div>
        {invoice.notes && <p className="text-[#94a3b8] text-xs italic border-t border-white/5 pt-3">{invoice.notes}</p>}
        ...
        {activity && activity.length > 0 && (
          <div className="space-y-1.5 border-t border-white/5 pt-3">
            <p className="text-[11px] font-bold uppercase text-[#94a3b8]">Activity</p>
            {activity.map((e) => (
            ...
            {expenses.map((e) => (
              <button key={e.id} onClick={() => setSelected(e)} className="w-full flex items-center justify-between gap-3 bg-black/20 border border-white/5 rounded-lg p-3 text-left hover:bg-white/5">
                <div className="min-w-0">
                  ...
                  <p className="text-[#94a3b8] text-[12px] font-mono mt-0.5">{e.category}</p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-white text-sm font-mono tabular-nums">${e.amount.toFixed(2)}</span>
  ...
  return (
    <Modal title="New expense" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <input value={vendor} onChange={(e) => setVendor(e.target.value)} required placeholder="Vendor" className="w-full bg-black/45 border border-white/15 rounded-lg px-3 py-2 text-sm text-white placeholder-[#8a96ab]" />
        <input value={category} onChange={(e) => setCategory(e.target.value)} required placeholder="Category" className="w-full bg-black/45 border border-white/15 rounded-lg px-3 py-2 text-sm text-white placeholder-[#8a96ab]" />
        <input value={amount} onChange={(e) => setAmount(e.target.value)} type="number" min="0.01" step="0.01" required placeholder="Amount (USD)" className="w-full bg-black/45 border border-white/15 rounded-lg px-3 py-2 text-sm text-white placeholder-[#8a96ab]" />
        <input value={expenseDate} onChange={(e) => setExpenseDate(e.target.value)} type="date" className="w-full bg-black/45 border border-white/15 rounded-lg px-3 py-2 text-sm text-white" />
        <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Description (optional)" rows={2} className="w-full bg-black/45 border border-white/15 rounded-lg px-3 py-2 text-sm text-white placeholder-[#8a96ab]" />
        {error && <p className="text-red-400 text-xs">{error}</p>}
...
function ExpenseDetailModal({ orgId, expense, onClose, onChanged }) {
  ...
  async function handleUploadReceipt(e) {
    ...
    try {
      ...
      await api(`/api/orgs/finance/expenses/${expense.id}/attachments`, { method: "POST", body: JSON.stringify({ orgId, filename: file.name, fileHash, sizeBytes: file.size, cidAlpha, cidBeta }) });
      setFile(null);
      setPasskey("");
    ...
    <Modal title={expense.vendor} onClose={onClose}>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-white text-lg font-mono tabular-nums">${expense.amount.toFixed(2)}</span>
        </div>
        <p className="text-[12px] font-mono text-[#94a3b8]">{expense.category} · {new Date(expense.expenseDate).toLocaleDateString()}</p>
        ...
        <div className="space-y-2 border-t border-white/5 pt-3">
          <p className="text-[11px] font-bold uppercase text-[#94a3b8]">Receipts</p>
            ...
            {payments.map((p) => (
              <div key={p.id} className="flex items-center justify-between gap-3 bg-black/20 border border-white/5 rounded-lg p-3">
                <div className="min-w-0">
                  ...
                  <p className="text-[#94a3b8] text-[12px] font-mono mt-0.5">{p.method || "—"} · {new Date(p.paymentDate).toLocaleDateString()}</p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
  ...
  return (
    <Modal title="Record payment" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <select value={direction} onChange={(e) => setDirection(e.target.value)} className="w-full bg-black/45 border border-white/15 rounded-lg px-2.5 py-2 text-xs text-white">
          <option value="INCOMING">Incoming</option>
          <option value="OUTGOING">Outgoing</option>
        </select>
        <input value={amount} onChange={(e) => setAmount(e.target.value)} type="number" min="0.01" step="0.01" required placeholder="Amount (USD)" className="w-full bg-black/45 border border-white/15 rounded-lg px-3 py-2 text-sm text-white placeholder-[#8a96ab]" />
        <input value={method} onChange={(e) => setMethod(e.target.value)} placeholder="Method (e.g. bank transfer)" className="w-full bg-black/45 border border-white/15 rounded-lg px-3 py-2 text-sm text-white placeholder-[#8a96ab]" />
        <input value={paymentDate} onChange={(e) => setPaymentDate(e.target.value)} type="date" className="w-full bg-black/45 border border-white/15 rounded-lg px-3 py-2 text-sm text-white" />
        {error && <p className="text-red-400 text-xs">{error}</p>}
...
function ReportsTab({ orgId }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {error && <p className="text-red-400 text-xs">{error}</p>}
      <div className="bg-[#090d16]/80 border border-white/5 rounded-2xl p-5">
        {!report ? <p className="text-[#94a3b8] font-mono text-sm">Loading…</p> : report.rows.length === 0 ? (
          <EmptyState compact icon="📊" description="No data for this report yet." />
        ) : (
          <div className="space-y-3">
            <p className="text-white text-sm font-mono tabular-nums">{report.count} record{report.count === 1 ? "" : "s"} · Total ${report.totalAmount.toFixed(2)}</p>
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead><tr className="text-[#94a3b8] uppercase text-[10px]">{Object.keys(report.rows[0]).map((k) => <th key={k} className="text-left py-1.5 pr-4 font-bold">{k}</th>)}</tr></thead>
                <tbody>
...
function Modal({ title, onClose, children, wide }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className={`bg-[#090d16] border border-white/10 rounded-2xl p-5 w-full ${wide ? "max-w-lg" : "max-w-md"} max-h-[85vh] overflow-y-auto`}>
