...
const STATUS_COLORS = {
  ...
  TERMINATED: "bg-white/5 text-[var(--inaya-text-muted)] border-white/10",
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search employees…" className="bg-black/45 border border-white/15 rounded-lg px-3 py-2 text-xs text-[var(--inaya-text-primary)] placeholder-[#8a96ab] w-56" />
      ...
      <div className="bg-[var(--inaya-surface)] border border-white/5 rounded-2xl p-5">
        {!employees ? <p className="text-[var(--inaya-text-muted)] font-mono text-sm">Loading…</p> : employees.length === 0 ? (
          <EmptyState compact icon="🧑‍💼" description="No employee records visible yet." ctaLabel="Create one" onCta={() => setShowCreate(true)} />
        ) : filtered.length === 0 ? (
          <p className="text-[var(--inaya-text-muted)] text-xs">No employees match "{search}".</p>
        ) : (
            ...
            {filtered.map((emp) => (
              <button key={emp.id} onClick={() => setSelected(emp)} className="w-full flex items-center justify-between gap-3 bg-black/20 border border-white/5 rounded-lg p-3 text-left hover:bg-white/5">
                <div className="min-w-0">
                  <span className="text-[var(--inaya-text-primary)] text-sm">{emp.fullName}</span>
                  {emp.jobTitle && <p className="text-[var(--inaya-text-muted)] text-[12px] font-mono mt-0.5">{emp.jobTitle}</p>}
                </div>
  ...
  return (
    <Modal title="New employee" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <input value={fullName} onChange={(e) => setFullName(e.target.value)} required placeholder="Full name" className="w-full bg-black/45 border border-white/15 rounded-lg px-3 py-2 text-sm text-[var(--inaya-text-primary)] placeholder-[#8a96ab]" />
        <input value={jobTitle} onChange={(e) => setJobTitle(e.target.value)} placeholder="Job title (optional)" className="w-full bg-black/45 border border-white/15 rounded-lg px-3 py-2 text-sm text-[var(--inaya-text-primary)] placeholder-[#8a96ab]" />
        <input value={memberEmail} onChange={(e) => setMemberEmail(e.target.value)} type="email" placeholder="Linked workspace login (optional)" className="w-full bg-black/45 border border-white/15 rounded-lg px-3 py-2 text-sm text-[var(--inaya-text-primary)] placeholder-[#8a96ab]" />
        <input value={joiningDate} onChange={(e) => setJoiningDate(e.target.value)} type="date" className="w-full bg-black/45 border border-white/15 rounded-lg px-3 py-2 text-sm text-[var(--inaya-text-primary)]" />
        {error && <p className="text-red-400 text-xs">{error}</p>}
      ...
      <div className="space-y-4">
        ...
        {balance && (
          <div className="border-t border-white/5 pt-3 flex items-center justify-between text-xs">
            <span className="text-[var(--inaya-text-muted)] uppercase font-bold">Leave balance</span>
            <span className="text-[var(--inaya-text-primary)] font-mono tabular-nums">{balance.remainingDays} / {balance.allocationDays} days</span>
          </div>
        )}
          ...
          <div className="space-y-2">
            {requests.map((r) => (
              <div key={r.id} className="flex items-center justify-between gap-3 bg-black/20 border border-white/5 rounded-lg p-3">
                <div className="min-w-0">
                  <span className="text-[var(--inaya-text-primary)] text-sm">{r.leaveType}</span>
                  <p className="text-[var(--inaya-text-muted)] text-[12px] font-mono mt-0.5">{new Date(r.startDate).toLocaleDateString()} – {new Date(r.endDate).toLocaleDateString()}</p>
                </div>
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  {r.status === "PENDING" && (
                    <>
                      ...
                      <ConfirmButton onConfirm={() => handleAction(r.id, "cancel")} disabled={!!acting} className="text-[11px] font-bold uppercase text-[var(--inaya-text-muted)] bg-white/5 border border-white/15 px-2 py-1 rounded-lg disabled:opacity-40">Cancel</ConfirmButton>
                    </>
                  )}
                </div>
              </div>
...
function CreateLeaveModal({ orgId, email, onClose, onCreated }) {
  ...
  return (
    <Modal title="Request leave" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-3">
        <select value={employeeId} onChange={(e) => setEmployeeId(e.target.value)} required className="w-full bg-black/45 border border-white/15 rounded-lg px-2.5 py-2 text-xs text-[var(--inaya-text-primary)]">
          <option value="">Employee…</option>
          {employees.map((emp) => <option key={emp.id} value={emp.id}>{emp.fullName}{emp.memberEmail === email ? " (you)" : ""}</option>)}
        </select>
        <select value={leaveType} onChange={(e) => setLeaveType(e.target.value)} className="w-full bg-black/45 border border-white/15 rounded-lg px-2.5 py-2 text-xs text-[var(--inaya-text-primary)]">
        ...
        <textarea value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Reason (optional)" rows={2} className="w-full bg-black/45 border border-white/15 rounded-lg px-3 py-2 text-sm text-[var(--inaya-text-primary)] placeholder-[#8a96ab]" />
        {error && <p className="text-red-400 text-xs">{error}</p>}
          ...
          <div className="space-y-3">
            {departments.map((d) => (
              <div key={d.id} className="bg-black/20 border border-white/5 rounded-lg p-3 space-y-2">
                <span className="text-[var(--inaya-text-primary)] text-sm">{d.name}</span>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-black/45 border border-white/15 rounded-lg px-2.5 py-1.5 text-xs text-[var(--inaya-text-primary)] placeholder-[#8a96ab]"
                  />
