...
export default function MfaVerifyScreen({ mfaPendingToken, onVerified, onCancel }) {
  ...
  async function handleSubmit(e) {
    ...
    setError("");
    try {
      await api("/api/orgs/mfa/verify", { method: "POST", body: JSON.stringify({ mfaPendingToken, code: code.trim() }) });
      onVerified();
    } catch (err) {
  ...
  async function handleSendSms() {
    setError("");
    try {
      await api("/api/orgs/mfa/send-sms", { method: "POST", body: JSON.stringify({ mfaPendingToken }) });
      setSmsSent(true);
    } catch (err) {
      setError(err.message);
    }
  ...
  return (
    <div className="max-w-sm mx-auto w-full inaya-fade-in-up">
      ...
      <button onClick={handleSendSms} disabled={smsSent} className="mt-4 text-xs text-[var(--inaya-text-muted)] hover:text-[#00f2fe] disabled:opacity-40 block mx-auto">
