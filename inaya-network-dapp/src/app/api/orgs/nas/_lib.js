// src/app/api/orgs/nas/_lib.js
//
// One wrapper for every NAS endpoint (SOW 42). In this order, for every route:
...
// enforces the NAS permission, validates input, audits, and never leaks
// secrets. Route files stay thin wrappers over src/lib/nas/*.
...
function toResponse(result, okStatus) {
  if (result?.error) {
    ...
    const safe = {};
    for (const k of ["runId", "jobId", "errors", "reasonCode", "sod", "differences", "state"]) if (rest[k] !== undefined) safe[k] = rest[k];
...
/**
 * nasRoute(handler, { okStatus }) -> a Next.js route handler.
 * handler receives { orgId, membership, email, query, params, body, req }.
 */
export function nasRoute(handler, { okStatus = 200 } = {}) {
  return async function route(req, ctx) {
    ...
    try {
      ...
      try {
        await checkRateLimit({ action: `nas:${mutating ? "write" : "read"}`, key: email, max: mutating ? 90 : 400, windowMs: 15 * 60 * 1000 });
      } catch {
      ...
      if (idemKey) {
        ...
        requests = (await getOrgCollections()).nasRequests;
      ...
      console.error(`nas route ${req.method} ${req.url} failed:`, err?.message || err);
