...
- **Terraform custom resource provider** (`inaya_volume`, `inaya_snapshot`, `inaya_backup_policy`, etc.) — no Inaya-authored Terraform provider exists at all today (what's tested is the stock `hashicorp/aws` provider against the S3-compat REST API). A real custom provider is a separate, multi-week Go effort, out of scope for this pass.
- **IBM Cloud Object Storage interop validation** — no IBM Cloud credentials exist in this environment; not executed, not simulated.
